...
arrayCivVeh =["LIB_zis5v","LIB_zis5v_med"];
...
campsTony = []; // list of current BM camps
...
server setVariable ["LIB_zis5v",300,true];//200
server setVariable ["LIB_zis5v_med",400,true];//600
server setVariable ["LIB_P39",6000,true];//12000
server setVariable ["LIB_kfz1",150,true];//50
server setVariable ["LIB_Scout_m3",400,true];//200
server setVariable ["LIB_US_GMC_Tent",300,true];//300
server setVariable ["LIB_SOV_M3_Halftrack",700,true];//700
{server setVariable [_x,400,true]} forEach ["B_HMG_01_high_F","B_G_Boat_Transport_01_F","B_G_Offroad_01_repair_F"];//400
{server setVariable [_x,800,true]} forEach ["LIB_BM37","LIB_Zis3","LIB_61k"];//800

server setVariable [vfs select 0,300,true];
server setVariable [vfs select 1,400,true];//600
server setVariable [vfs select 2,6000,true];//12000
server setVariable [vfs select 3,150,true];//50
server setVariable [vfs select 4,400,true];//200
server setVariable [vfs select 5,300,true];//300
...
if (hayRHS) then {
	...
	server setVariable [vfs select 12,1000,true];
...
server setVariable ["resourcesAAF",5000,true];//Initial AAF resources
...
server setVariable ["enableFTold",false,true];
server setVariable ["enableMemAcc",false,true];

server setVariable ["easyMode",false,true];
