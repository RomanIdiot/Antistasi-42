...
fn_SetStat =
{
	...
	{
		if(_varName == 'cuentaCA') then { // experimental
			if (cuentaCA < 0) then {
				cuentaCA = 0;
			}
			else {
		...
		if(_varname == 'estaticas') then
			...
			for "_i" from 0 to (count _varvalue) - 1 do
				...
				if (_tipoVeh in (allStatMGs + allStatATs + allStatAAs + allStatMortars)) then
					{
					diag_log format ["saveFuncs -- estaticas is static: %1; type: %2", _varvalue select _i,_tipoVeh];
